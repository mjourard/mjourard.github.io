...
{% include hot-reloading-of-local-typescript-lambdas/lambda-hot-reloading.gif %}
...
For example, looking at the example template above and the output, the value we care about is `functions.example.events[0].http.path` in the `serverless.yml` template, and the full endpoint would be:
`http://localhost:4570/restapis/2tfmo0ytms/local/_user_request_/hello`
When I navigate to that URL on my local, I get the following output:
```json
{ 
